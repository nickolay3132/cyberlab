...
class ConcurrentImporter:
    ...
    def _check_task(self, task: ImportVmTask):
        if task.is_completed:
            print(Fore.CYAN + "***" + f"{task.vm_name} successfully imported.".center(60) + "***")
        else:
            print(f"\n{Fore.RED}Error importing {task.vm_name}")
            if task.error:
    ...
    def _thread_handler(self, ova_path: str, vm_name: str, vms_dir: str):
        ...
        try:
            ...
            if success:
                SnapshotAdapter.create(vm_name, f"{self.timestamp}-initial-state",
